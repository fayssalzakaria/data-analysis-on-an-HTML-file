...
def compute_advanced_statistics(df):
    ...
    if response_col in df.columns:
        ...
        if not df_cluster.empty:
            ...
            df["Response Cluster"] = np.nan
            df_cluster["Response Cluster"] = kmeans.fit_predict(df_cluster)
            df.loc[df_cluster.index, "Response Cluster"] = df_cluster["Response Cluster"]
...
def generate_advanced_graphs(df, output_dir="graphs/"):
    ...
    if "Response Cluster" in df.columns:
        fig = px.scatter(
            df.dropna(subset=["Response Cluster", "Response Time (minutes)", age_col]),
            x="Response Time (minutes)",
            y=age_col,
            color="Response Cluster",
